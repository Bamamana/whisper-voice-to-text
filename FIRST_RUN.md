...
- Keep internet access available during setup so the Windows flow can bootstrap Python, FFmpeg, and the Visual C++ runtime automatically when they are missing.
...
Manual `setup_windows.bat` pre-downloads `tiny`, `base`, and `small` automatically.
The packaged installer skips optional model pre-downloads during install so first-run setup finishes more reliably.
