...
2. Keep internet access available during setup so the Windows flow can bootstrap Python, FFmpeg, and the Visual C++ runtime if needed.
...
Manual `setup_windows.bat` pre-downloads `tiny`, `base`, and `small` by default.
The packaged installer skips optional model pre-downloads during install so first setup is more reliable.
