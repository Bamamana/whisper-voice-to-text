...
class DriveCentricVoiceFormApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Whisper Voice To Form - V5 Drive Workspace")
        self.root.geometry("1400x850")

        self.app_dir = Path(__file__).resolve().parent
        self.api_key_file = self.app_dir / ".gemini-api-key"
        self.drive_settings_file = self.app_dir / ".v5-drive-settings.json"
        ...
        drive_settings = self._load_drive_settings()
        self.drive_type_var = tk.StringVar(value=drive_settings.get("drive_type", "Computer Drive"))
        self.drive_root_path_var = tk.StringVar(value=drive_settings.get("drive_root_path", ""))
        self.dropbox_parent_path_var = self.drive_root_path_var
        self.file_group_path_var = tk.StringVar(value="")
        self.active_matter_path_var = self.file_group_path_var
        ...
        self._configure_visual_planner_style()
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        top_bar = ttk.Frame(self.root, padding=(12, 10), style="Toolbar.TFrame")
        top_bar.pack(fill=tk.X, side=tk.TOP)

        ttk.Label(top_bar, text="Google AI Studio API key", style="Surface.TLabel").pack(side=tk.LEFT)
        ttk.Entry(top_bar, textvariable=self.api_key_var, show="*", width=34).pack(side=tk.LEFT, padx=(6, 4))
        ttk.Button(top_bar, text="Save Key", command=self.save_api_key, style="Emerald.TButton").pack(side=tk.LEFT)
        ttk.Button(top_bar, text="Test Key", command=self.test_api_key, style="Sky.TButton").pack(side=tk.LEFT, padx=(4, 12))
        ttk.Label(top_bar, text="Gemini", style="Surface.TLabel").pack(side=tk.LEFT)
        ttk.Entry(top_bar, textvariable=self.gemini_model_var, width=22).pack(side=tk.LEFT, padx=(6, 12))
        ttk.Label(top_bar, text="Whisper", style="Surface.TLabel").pack(side=tk.LEFT)
        ttk.Combobox(
            top_bar,
        ...
        ttk.Checkbutton(top_bar, text="Large Controls", variable=self.accessibility_mode_var, command=self.toggle_accessibility_mode).pack(side=tk.LEFT)

        action_bar = ttk.Frame(self.root, padding=(12, 8), style="Toolbar.TFrame")
        action_bar.pack(fill=tk.X, side=tk.TOP)
        ttk.Button(action_bar, text="Start Recording (F5)", command=self.start_recording, style="Primary.TButton").pack(side=tk.LEFT)
        ttk.Button(action_bar, text="Stop and Transcribe (F6)", command=self.stop_recording, style="Rose.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(action_bar, text="Fill Loaded Form (F8)", command=self.send_transcript_to_ai, style="Indigo.TButton").pack(side=tk.LEFT, padx=(14, 0))
        ttk.Button(action_bar, text="Export Filled PDF (F9)", command=self.export_filled_pdf, style="Sky.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(action_bar, text="Update Original PDF (F10)", command=self.update_original_pdf, style="Amber.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(action_bar, text="Shortcut Help", command=self.show_command_help).pack(side=tk.RIGHT)

        status_line = ttk.Frame(self.root, padding=(12, 4), style="Status.TFrame")
        status_line.pack(fill=tk.X, side=tk.TOP)
        ttk.Label(status_line, textvariable=self.pdf_status_var, style="Muted.TLabel").pack(side=tk.LEFT)
        ttk.Label(status_line, textvariable=self.recording_status_var, style="Muted.TLabel").pack(side=tk.RIGHT)

        workspace = ttk.PanedWindow(self.root, orient=tk.HORIZONTAL)
        workspace.pack(fill=tk.BOTH, expand=True, padx=10, pady=(8, 6))

        left_workspace = ttk.Frame(workspace, style="Surface.TFrame")
        preview = ttk.LabelFrame(workspace, text="Document Viewport", padding=8)
        workspace.add(left_workspace, weight=1)
        workspace.add(preview, weight=2)

        self.notebook = ttk.Notebook(left_workspace)
        self.notebook.pack(fill=tk.BOTH, expand=True)
        self.tab_files = ttk.Frame(self.notebook, padding=10)
        self.tab_voice = ttk.Frame(self.notebook, padding=10)
        self.tab_email = ttk.Frame(self.notebook, padding=10)
        self.tab_fields = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(self.tab_files, text="File Hub")
        self.notebook.add(self.tab_voice, text="Dictation & Legal")
        self.notebook.add(self.tab_email, text="Email Center")
        self.notebook.add(self.tab_fields, text="Form Fields")

        drive_setup = ttk.LabelFrame(self.tab_files, text="Drive Workspace", padding=10)
        drive_setup.pack(fill=tk.X, pady=(0, 8))
        ttk.Combobox(
            drive_setup,
            textvariable=self.drive_type_var,
            values=["Computer Drive", "Dropbox", "OneDrive", "Google Drive", "Local / Network"],
            state="readonly",
            width=16,
        ).grid(row=0, column=0, sticky="w")
        ttk.Entry(drive_setup, textvariable=self.drive_root_path_var).grid(row=0, column=1, sticky="ew", padx=(6, 6))
        ttk.Button(drive_setup, text="Choose Root", command=self.select_drive_root_folder, style="Primary.TButton").grid(row=0, column=2, sticky="ew")
        ttk.Button(drive_setup, text="Refresh", command=self.refresh_matter_directories, style="Emerald.TButton").grid(row=0, column=3, sticky="ew", padx=(6, 0))
        drive_setup.columnconfigure(1, weight=1)

        file_splitter = ttk.PanedWindow(self.tab_files, orient=tk.VERTICAL)
        file_splitter.pack(fill=tk.BOTH, expand=True)

        matter_panel = ttk.Frame(file_splitter)
        file_splitter.add(matter_panel, weight=1)
        ttk.Label(matter_panel, text="Matter / client folder").grid(row=0, column=0, sticky="w")
        self.matter_folder_combo = ttk.Combobox(matter_panel, state="readonly")
        self.dropbox_folder_combo = self.matter_folder_combo
        self.matter_folder_combo.grid(row=0, column=1, sticky="ew", padx=(6, 6))
        self.matter_folder_combo.bind("<<ComboboxSelected>>", lambda _event: self.load_selected_matter_directory())
        ttk.Button(matter_panel, text="Load Folder", command=self.load_selected_matter_directory, style="Emerald.TButton").grid(row=0, column=2, sticky="ew")
        ttk.Button(matter_panel, text="Choose Specific Folder", command=self.select_matter_file_group, style="Sky.TButton").grid(row=0, column=3, sticky="ew", padx=(6, 0))

        profile_box = ttk.LabelFrame(matter_panel, text="Matter Profile", padding=8)
        profile_box.grid(row=1, column=0, columnspan=4, sticky="ew", pady=(8, 0))
        ttk.Label(profile_box, text="Client").grid(row=0, column=0, sticky="w")
        ttk.Entry(profile_box, textvariable=self.client_name_var).grid(row=0, column=1, sticky="ew", padx=(6, 8))
        ttk.Label(profile_box, text="Matter #").grid(row=0, column=2, sticky="w")
        ttk.Entry(profile_box, textvariable=self.matter_number_var, width=18).grid(row=0, column=3, sticky="ew", padx=(6, 0))
        ttk.Label(profile_box, text="Court").grid(row=1, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(profile_box, textvariable=self.court_var).grid(row=1, column=1, sticky="ew", padx=(6, 8), pady=(6, 0))
        ttk.Label(profile_box, text="Opposing").grid(row=1, column=2, sticky="w", pady=(6, 0))
        ttk.Entry(profile_box, textvariable=self.opposing_party_var, width=18).grid(row=1, column=3, sticky="ew", padx=(6, 0), pady=(6, 0))
        ttk.Label(profile_box, text="Saved profile").grid(row=2, column=0, sticky="w", pady=(8, 0))
        self.matter_profile_combo = ttk.Combobox(profile_box, textvariable=self.matter_profile_var, state="readonly")
        self.matter_profile_combo.grid(row=2, column=1, sticky="ew", padx=(6, 8), pady=(8, 0))
        ttk.Button(profile_box, text="Load", command=self.load_selected_matter_profile).grid(row=2, column=2, sticky="ew", pady=(8, 0))
        profile_actions = ttk.Frame(profile_box)
        profile_actions.grid(row=2, column=3, sticky="ew", padx=(6, 0), pady=(8, 0))
        ttk.Button(profile_actions, text="Extract From Dictation", command=self.extract_matter_profile_from_transcript, style="Indigo.TButton").pack(side=tk.LEFT)
        ttk.Button(profile_actions, text="Save Profile", command=self.save_current_matter_profile, style="Emerald.TButton").pack(side=tk.LEFT, padx=(6, 0))
        profile_box.columnconfigure(1, weight=1)
        profile_box.columnconfigure(3, weight=1)
        matter_panel.columnconfigure(1, weight=1)

        documents_panel = ttk.LabelFrame(file_splitter, text="Documents in Active Matter", padding=8)
        file_splitter.add(documents_panel, weight=1)
        file_list_frame = ttk.Frame(documents_panel)
        file_list_frame.pack(fill=tk.BOTH, expand=True)
        self.matter_files_listbox = tk.Listbox(file_list_frame, height=8)
        self.matter_files_listbox.configure(
            background=self.colors["panel"],
            foreground=self.colors["text"],
            selectbackground=self.colors["emerald_soft"],
        ...
        document_actions = ttk.Frame(documents_panel)
        document_actions.pack(fill=tk.X, pady=(6, 0))
        ttk.Button(document_actions, text="Open Selected", command=self.open_selected_matter_file, style="Primary.TButton").pack(side=tk.LEFT)
        ttk.Button(document_actions, text="Auto-Prepare Selected PDF", command=self.auto_prepare_selected_pdf, style="Amber.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(document_actions, text="Upload PDF", command=self.load_pdf, style="Sky.TButton").pack(side=tk.RIGHT)

        transcript_header = ttk.Frame(self.tab_voice)
        transcript_header.pack(fill=tk.X)
        ttk.Label(transcript_header, text="Transcript", style="Header.TLabel").pack(side=tk.LEFT)
        ttk.Button(transcript_header, text="Copy", command=self.copy_transcript, style="Emerald.TButton").pack(side=tk.LEFT, padx=(8, 0))
        ttk.Button(transcript_header, text="Fix Grammar", command=lambda: self.rewrite_transcript_with_gemini("grammar"), style="Indigo.TButton").pack(side=tk.RIGHT)
        ttk.Button(transcript_header, text="Make Email", command=lambda: self.rewrite_transcript_with_gemini("email"), style="Sky.TButton").pack(side=tk.RIGHT, padx=(0, 6))
        self.transcript_text = tk.Text(self.tab_voice, wrap=tk.WORD, height=12)
        self.transcript_text.configure(background=self.colors["panel"], foreground=self.colors["text"], insertbackground=self.colors["text"], relief=tk.FLAT, borderwidth=1, highlightthickness=1, highlightbackground=self.colors["border"])
        self.transcript_text.pack(fill=tk.BOTH, expand=True, pady=(6, 8))

        legal_tools = ttk.LabelFrame(self.tab_voice, text="Legal Work Product", padding=8)
        legal_tools.pack(fill=tk.X)
        ttk.Button(legal_tools, text="Formal Legal Tone", command=lambda: self.create_legal_work_product("formal_tone"), style="Indigo.TButton").grid(row=0, column=0, sticky="ew", padx=2, pady=2)
        ttk.Button(legal_tools, text="Attorney Notes", command=lambda: self.create_legal_work_product("attorney_notes"), style="Indigo.TButton").grid(row=0, column=1, sticky="ew", padx=2, pady=2)
        ttk.Button(legal_tools, text="Billing Entry", command=lambda: self.create_legal_work_product("billing_entry"), style="Amber.TButton").grid(row=0, column=2, sticky="ew", padx=2, pady=2)
        ttk.Button(legal_tools, text="Timeline", command=lambda: self.create_legal_work_product("timeline"), style="Sky.TButton").grid(row=1, column=0, sticky="ew", padx=2, pady=2)
        ttk.Button(legal_tools, text="Client Letter", command=lambda: self.create_legal_work_product("client_letter"), style="Emerald.TButton").grid(row=1, column=1, sticky="ew", padx=2, pady=2)
        ttk.Button(legal_tools, text="Checklist", command=lambda: self.create_legal_work_product("checklist"), style="Amber.TButton").grid(row=1, column=2, sticky="ew", padx=2, pady=2)
        for column in range(3):
            legal_tools.columnconfigure(column, weight=1)
        ttk.Label(self.tab_voice, text="Output").pack(anchor="w", pady=(8, 0))
        self.legal_output_text = tk.Text(self.tab_voice, wrap=tk.WORD, height=8)
        self.legal_output_text.configure(background=self.colors["panel"], foreground=self.colors["text"], insertbackground=self.colors["text"], relief=tk.FLAT, borderwidth=1, highlightthickness=1, highlightbackground=self.colors["border"])
        self.legal_output_text.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        email_fields = ttk.Frame(self.tab_email)
        ...
        ttk.Label(self.tab_email, text="Email body").pack(anchor="w", pady=(8, 0))
        self.email_body_text = tk.Text(self.tab_email, wrap=tk.WORD, height=12)
        self.email_body_text.configure(background=self.colors["panel"], foreground=self.colors["text"], insertbackground=self.colors["text"], relief=tk.FLAT, borderwidth=1, highlightthickness=1, highlightbackground=self.colors["border"])
        self.email_body_text.pack(fill=tk.BOTH, expand=True, pady=(4, 6))
        email_buttons = ttk.Frame(self.tab_email)
        email_buttons.pack(fill=tk.X)
        ttk.Button(email_buttons, text="Generate From Dictation", command=lambda: self.rewrite_transcript_with_gemini("email"), style="Sky.TButton").pack(side=tk.LEFT)
        ttk.Button(email_buttons, text="Connect Gmail", command=self.connect_gmail, style="Emerald.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(email_buttons, text="Create Gmail Draft", command=self.create_gmail_draft, style="Primary.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(email_buttons, text="Open Email App", command=self.open_email_app, style="Sky.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(email_buttons, text="Gmail Setup Help", command=self.show_gmail_setup_help).pack(side=tk.RIGHT)

        fields_actions = ttk.Frame(self.tab_fields)
        fields_actions.pack(fill=tk.X, pady=(0, 6))
        ttk.Button(fields_actions, text="Review Fields With Gemini", command=self.review_fields_with_gemini, style="Indigo.TButton").pack(side=tk.LEFT)
        ttk.Button(fields_actions, text="Save As Template", command=self.save_current_as_template, style="Emerald.TButton").pack(side=tk.LEFT, padx=(6, 0))
        ttk.Label(fields_actions, text="Template").pack(side=tk.LEFT, padx=(12, 4))
        self.template_combo = ttk.Combobox(fields_actions, textvariable=self.template_var, width=24, state="readonly")
        self.template_combo.pack(side=tk.LEFT)
        ttk.Button(fields_actions, text="Load Template", command=self.load_selected_template, style="Sky.TButton").pack(side=tk.LEFT, padx=(6, 0))
        self.fields_tree = ttk.Treeview(self.tab_fields, columns=("value",), show="tree headings")
        self.fields_tree.heading("#0", text="Field")
        self.fields_tree.heading("value", text="Value")
        self.fields_tree.column("#0", width=240)
        ...
        self.preview_canvas.configure(background=self.colors["panel_alt"], highlightthickness=1, highlightbackground=self.colors["border"])
        ...
        bottom_strip = ttk.Frame(self.root, padding=(12, 5), style="Status.TFrame")
        bottom_strip.pack(fill=tk.X, side=tk.BOTTOM)
        ttk.Label(bottom_strip, textvariable=self.status_var, style="Muted.TLabel").pack(anchor="w")
    ...
    def _load_drive_settings(self) -> dict[str, str]:
        if not self.drive_settings_file.exists():
            return {}
        try:
            data = json.loads(self.drive_settings_file.read_text(encoding="utf-8"))
        except Exception:
            return {}
    ...
    def _save_drive_settings(self) -> None:
        data = {
            "drive_type": self.drive_type_var.get().strip() or "Computer Drive",
            "drive_root_path": self.drive_root_path_var.get().strip(),
        }
        self.drive_settings_file.write_text(json.dumps(data, indent=2), encoding="utf-8")

    def _configure_visual_planner_style(self) -> None:
        self.colors = {
            "canvas": "#f8fafc",
            "panel": "#ffffff",
            "panel_alt": "#f1f5f9",
        ...
        style.configure(".", font=("Segoe UI", 10), background=self.colors["canvas"], foreground=self.colors["text"])
        style.configure("TFrame", background=self.colors["canvas"])
        style.configure("Surface.TFrame", background=self.colors["panel"], borderwidth=1, relief="solid")
        style.configure("Toolbar.TFrame", background=self.colors["panel"], borderwidth=1, relief="solid")
        style.configure("Status.TFrame", background=self.colors["panel_alt"])
        style.configure("TLabel", background=self.colors["canvas"], foreground=self.colors["text"])
        style.configure("Muted.TLabel", background=self.colors["canvas"], foreground=self.colors["muted"])
        style.configure("Surface.TLabel", background=self.colors["panel"], foreground=self.colors["text"])
        style.configure("MutedSurface.TLabel", background=self.colors["panel"], foreground=self.colors["muted"])
        style.configure("Header.TLabel", background=self.colors["panel"], foreground=self.colors["text"], font=("Segoe UI", 12, "bold"))
        ...
        style.configure("TEntry", fieldbackground=self.colors["panel"], bordercolor=self.colors["border_strong"], lightcolor=self.colors["border"], darkcolor=self.colors["border"], padding=(8, 5))
        style.configure("TCombobox", fieldbackground=self.colors["panel"], background=self.colors["panel"], bordercolor=self.colors["border_strong"], arrowcolor=self.colors["muted"], padding=(8, 5))

        style.configure("TButton", background=self.colors["panel"], foreground=self.colors["text"], bordercolor=self.colors["border_strong"], focusthickness=1, focuscolor=self.colors["emerald"], padding=(10, 6), relief="flat")
        style.map("TButton", background=[("active", self.colors["panel_alt"]), ("pressed", self.colors["border"])])
        style.configure("Primary.TButton", background=self.colors["emerald"], foreground="#ffffff", bordercolor=self.colors["emerald"], padding=(12, 7), font=("Segoe UI", 10, "bold"))
        style.map("Primary.TButton", background=[("active", self.colors["emerald_hover"]), ("pressed", self.colors["emerald_hover"])], foreground=[("disabled", "#d1d5db")])
        style.configure("Emerald.TButton", background=self.colors["emerald_soft"], foreground="#065f46", bordercolor="#a7f3d0", padding=(10, 6), font=("Segoe UI", 9, "bold"))
        style.configure("Indigo.TButton", background=self.colors["indigo_soft"], foreground="#3730a3", bordercolor="#c7d2fe", padding=(10, 6), font=("Segoe UI", 9, "bold"))
        style.configure("Sky.TButton", background=self.colors["sky_soft"], foreground="#075985", bordercolor="#bae6fd", padding=(10, 6), font=("Segoe UI", 9, "bold"))
        style.configure("Amber.TButton", background=self.colors["amber_soft"], foreground="#92400e", bordercolor="#fde68a", padding=(10, 6), font=("Segoe UI", 9, "bold"))
        style.configure("Rose.TButton", background=self.colors["rose_soft"], foreground="#be123c", bordercolor="#fecdd3", padding=(10, 6), font=("Segoe UI", 9, "bold"))

        style.configure("TNotebook", background=self.colors["canvas"], borderwidth=0)
        style.configure("TNotebook.Tab", background=self.colors["panel_alt"], foreground=self.colors["muted"], padding=(14, 8), font=("Segoe UI", 9, "bold"))
        style.map("TNotebook.Tab", background=[("selected", self.colors["panel"]), ("active", self.colors["emerald_soft"])], foreground=[("selected", self.colors["emerald"])])
        style.configure("Treeview", background=self.colors["panel"], fieldbackground=self.colors["panel"], foreground=self.colors["text"], bordercolor=self.colors["border"], rowheight=26)
        style.configure("Treeview.Heading", background=self.colors["panel_alt"], foreground=self.colors["text"], font=("Segoe UI", 9, "bold"), relief="flat")
        style.map("Treeview", background=[("selected", self.colors["emerald_soft"])], foreground=[("selected", self.colors["text"])])
    ...
    def select_drive_root_folder(self) -> None:
        folder = filedialog.askdirectory(title=f"Choose {self.drive_type_var.get()} root folder")
        if not folder:
            return
        self.drive_root_path_var.set(folder)
        self._save_drive_settings()
        self.refresh_matter_directories()
        self.status_var.set(f"Drive root selected: {folder}")

    def refresh_matter_directories(self) -> None:
        self.refresh_dropbox_matter_folders()

    def load_selected_matter_directory(self) -> None:
        self.load_selected_dropbox_matter_folder()
    ...
    def refresh_dropbox_matter_folders(self) -> None:
        ...
        if not parent.exists() or not parent.is_dir():
            messagebox.showerror("Missing parent folder", "Choose an existing drive root folder first.")
            return
    ...
    def load_selected_dropbox_matter_folder(self) -> None:
        ...
        if not parent_text or not folder_name:
            messagebox.showerror("Missing folder", "Choose a drive root folder and a matter folder first.")
            return
    ...
    def load_matter_folder(self, folder: Path) -> None:
        ...
        if saved_profile:
            ...
            self.file_group_path_var.set(str(folder))
            self.status_var.set(f"Loaded matter profile from: {folder.name}")
        else:
    ...
    def load_profile_from_matter_folder(self, folder: Path) -> dict | None:
        ...
        try:
            ...
            messagebox.showwarning("Profile read failed", f"Could not read matter profile file:\n{profile_path}\n\n{exc}")
            return None
    ...
    def save_profile_to_matter_folder(self) -> Path:
        ...
        if not folder_text:
            raise RuntimeError("Choose or load a matter folder before saving the profile.")
        ...
        data["drive_type"] = self.drive_type_var.get().strip()
        data["drive_root_path"] = self.drive_root_path_var.get().strip()
    ...
    def open_pdf_from_matter_folder(self, file_path: Path) -> None:
        ...
        if fields:
            ...
            self._select_notebook_tab("fields")
            self.status_var.set(f"Loaded matter PDF in V5: {file_path.name}")
            return

        self.auto_prepare_and_review_pdf(file_path, "Matter PDF")

    def auto_prepare_selected_pdf(self) -> None:
        folder_text = self.file_group_path_var.get().strip()
        selected = self.matter_files_listbox.curselection() if hasattr(self, "matter_files_listbox") else ()
        if not folder_text or not selected:
            messagebox.showerror("No PDF selected", "Select a PDF from the active matter folder first.")
            return
        source_path = Path(folder_text) / self.matter_files_listbox.get(selected[0])
        if source_path.suffix.lower() != ".pdf":
            messagebox.showerror("Not a PDF", "Select a PDF file before auto-preparing a template.")
            return
        self.auto_prepare_and_review_pdf(source_path, "Selected PDF")

    def _select_notebook_tab(self, tab_name: str) -> None:
        if not hasattr(self, "notebook"):
            return
        tab = getattr(self, f"tab_{tab_name}", None)
        if tab is not None:
    ...
    def load_pdf(self, path: str | Path | None = None) -> None:
        ...
        if not fields:
            self.auto_prepare_and_review_pdf(Path(path), "Uploaded PDF")
            return
    ...
    def auto_prepare_and_review_pdf(self, source_path: Path, source_label: str = "PDF") -> None:
        output_path = self.templates_dir / f"{source_path.stem}-auto-template.pdf"
        try:
            field_count = self._create_fillable_template_from_flat_pdf(source_path, output_path)
        except Exception as exc:
        ...
        if field_count == 0:
            messagebox.showwarning(
                "No fields detected",
                "This PDF does not contain fillable fields, and V5 could not detect obvious blank lines or checkbox squares. "
                "This PDF may need manual preparation in Adobe Acrobat.",
            )
            return
        ...
        self._select_notebook_tab("fields")
        self.status_var.set(f"Auto-prepared {source_label.lower()} with {field_count} fields: {output_path.name}")

        if self.api_key_var.get().strip():
            self.review_fields_with_gemini()
        else:
            messagebox.showinfo(
                "Template prepared",
                "V5 auto-prepared this PDF and loaded the detected fields. Enter your Gemini API key and click "
                "Review Fields With Gemini to rename fields and detect obvious missing fields.",
            )
        ...
        if folder_text:
            ...
            if folder.exists() and folder.is_dir():
                ...
                if profile:
                    ...
                    self.file_group_path_var.set(str(folder))
                    self.refresh_matter_files()
                    self.status_var.set(f"Loaded folder profile: {folder.name}")
                    return

        profile_name = self.matter_profile_var.get()
        if not profile_name:
            messagebox.showerror("No profile", "No matter profile exists in the active folder, and no local saved profile is selected.")
            return
        try:
    ...
    def save_current_matter_profile(self) -> None:
        ...
        if folder_text:
            try:
                ...
                messagebox.showerror("Matter profile save failed", str(exc))
                return
            self.status_var.set(f"Matter profile saved in folder: {profile_path.name}")
            return
    ...
    def _current_matter_profile(self) -> dict[str, str]:
        return {
            ...
            "drive_type": self.drive_type_var.get().strip(),
            "drive_root_path": self.drive_root_path_var.get().strip(),
    ...
    def _apply_matter_profile(self, profile: dict, preserve_existing: bool = False) -> None:
        fields = [
            ...
            (self.drive_type_var, "drive_type"),
            (self.drive_root_path_var, "drive_root_path"),
        ...
        for variable, key in fields:
            ...
            if key == "drive_type" and not value:
                continue
            if key == "drive_root_path" and not value:
                continue
            if key == "dropbox_parent_path" and not value:
                continue
    ...
    def show_command_help(self) -> None:
        ...
        help_window.title("V5 Shortcut Help")
...
5. Use Export Filled PDF for a separate copy, or Update Original PDF to update the loaded PDF after V5 creates a backup.
...
Step 8 - Put the file in the V5 app folder
...
"""
    ...
    def _get_gmail_service(self):
        try:
            ...
            raise RuntimeError("Gmail dependencies are missing. Re-run install_windows_v5.bat.") from exc
        ...
        if not credentials or not credentials.valid:
            if not self.gmail_credentials_file.exists():
                raise RuntimeError(
                    ...
                    "download its JSON file, rename it to gmail-credentials.json, and place it in the V5 app folder."
                )
    ...
    def update_original_pdf(self) -> None:
        ...
        confirmed = messagebox.askyesno(
            ...
            "V5 will make a timestamped backup first. Continue?",
        )
        if not confirmed:
            return
        ...
        temp_path = original_path.with_name(f".{original_path.stem}.v5-update-{timestamp}{original_path.suffix}")
