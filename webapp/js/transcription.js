// Transcription backends: local bridge, OpenAI-compatible, Gemini.
...
// --- Local Python bridge (faster-whisper) ---

async function transcribeWithLocal(config, blob, extension, timestamps) {
  const endpoint = `${normalizeBaseUrl(config.baseUrl)}/transcribe`;
  ...
  const json = await ensureJsonResponse(response, 'Local Whisper request');
...
// --- OpenAI-compatible (Lemonade, OpenRouter, OpenAI, LM Studio, etc.) ---
...
export async function transcribeBlob(blob, extension, settings, timestamps) {
  const config = resolveProviderConfig(settings);
  const transport = getTransport(config.provider);

  if (transport === 'local') {
    return transcribeWithLocal(config, blob, extension, timestamps);
  }
  if (transport === 'gemini') {
...
// --- Model discovery (dynamic dropdown) ---
...
export async function discoverModels(settings) {
  ...
  if (transport === 'local') {
    ...
    const json = await ensureJsonResponse(response, 'Local model discovery');
    return (json.models || []).map((entry) => ({ id: entry.id || entry, label: entry.label || entry.id || entry }));
  }

  if (transport === 'gemini') {
    const endpoint = `${normalizeBaseUrl(config.baseUrl)}/models?key=${config.apiKey}`;
    const json = await ensureJsonResponse(response2(endpoint), 'Gemini model discovery');
    return (json.models || [])
...
async function response2(endpoint) {
